...
class CARAFCNAgent(Agent):
    ...
    def _calc_temporal_weights(
        ...
        if self.is_adaptive:
            ...
            #    max(
            #        0,
            #        self.w_f + self.learning_rate * np.sign(pred_r_f * obs_r_f)
            #    )
            #)
            #w_c = min(
            #    self.w_c_max,
            #    max(
            #        0,
            #        self.w_c + self.learning_rate * np.sign(pred_r_c * obs_r_c)
            #    )
            #)
            w_f = min(
    ...
    def _update_weight(
        ...
        if pred_acc == 1:
            ...
            updated_w: float = current_w - self.learning_rate * np.abs(pred_r+obs_r)
        elif pred_acc == 0:
